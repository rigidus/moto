...
** COMMENT Страница списка вакансий
   ...
   Теперь нам необходим оборачивающий блок, который поместит список вакансий в
   соответствующий ему блок:

   #+NAME: vac_elt_list_block
   #+BEGIN_SRC lisp
     (in-package #:moto)

     (defun vac-elt-list-block (vacs vac-type)
       (append (list (format nil "col-~A" vac-type) vac-type)
               (vac-elt-list vacs vac-type)))

     ;; (vac-elt-list-block (last (all-vacancy) 2) "unsort")
   ...
   И теперь мы можем фильтровать вакансии в соответствии с их типом:

   #+NAME: vac_elt_list_block_fltr
   #+BEGIN_SRC lisp
     (in-package #:moto)

     (defun vac-elt-list-block-fltr (vacs vac-type)
       (let* ((target-state (format nil ":~A" (string-upcase vac-type)))
              (filtered-vacs (remove-if-not #'(lambda (vac)
                                                (equal (state vac) target-state))
                                            vacs)))
         (vac-elt-list-block filtered-vacs vac-type)))
   #+END_SRC
         ...
         (html-page
          `(,@(legend)
              ...
              ("div" (("class" "row no-gutters"))
                     ,@(apply #'vac-col (vac-elt-list-block-fltr sorted-vacs "uninteresting"))
                     ,@(apply #'vac-col (vac-elt-list-block-fltr sorted-vacs "unsort"))
