...
** Страница списка вакансий
   ...
   #+BEGIN_SRC lisp
     ...
     (restas:define-route hhtest ("/hh/test")
       (let* ((vacs (aif (all-vacancy) it (err "null vacancy")))
              (sorted-vacs (sort vacs #'sort-vacancy-by-salary)))
         (flet ((show-vac-elt (vac class title noteclass)
                  (vac-elt (src-id vac) "unsort" "" "emptynotes" (pretty-salary vac) (name vac))))
           (concatenate
            'string
            "<!DOCTYPE html>"
            (format nil "~%")
                  ...
                  ("div" (("class" "row no-gutters"))
                         ,@(apply
                            ...
                            (append (list "col-uninteresting" "uninteresting")
                                    (let ((filtered-vacs (remove-if-not #'(lambda (vac) (equal (state vac) ":UNINTERESTING")) sorted-vacs)))
                                      (if filtered-vacs
                                          (mapcar #'(lambda (vac) (show-vac-elt vac "uninteresting" "" "emptynotes"))
                                                  filtered-vacs)
                                          (list
                         ...
                         ,@(apply
                            ...
                            (append (list "col-unsort" "unsort")
                                    (let* ((filtered-vacs (remove-if-not #'(lambda (vac) (equal (state vac) ":UNSORT")) sorted-vacs)))
                                      (if filtered-vacs
                                          (mapcar #'(lambda (vac) (show-vac-elt vac "unsort" "" "emptynotes"))
                                                  filtered-vacs)
                                          (list
                                           (vac-elt 22604660 "unsort" "" "emptynotes" "emptynotes" "DYMMY"))))))
