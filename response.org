...
#+BEGIN_SRC lisp
  ...
  ;;            (let* ((src-id (src-id vac))
  ;;                   (vacancy-page (hh-get-page (format nil "https://hh.ru/vacancy/~A" src-id)
  ;;                                              cookie-jar
  ;;                                              *hh_account*
  ;;                                              "https://spb.hh.ru/"))
  ;;                   (archive-p (search "Вакансия в архиве" vacancy-page)))
  ;;              (if archive-p
  ;;                  (upd-vacancy vac (list :state  ":UNINTERESTING"))))))
  ;;     (mapcar #'uninteresting-if-archive
  ;;             (all-vacancy))))
