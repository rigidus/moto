...
** START Жизненный цикл резюме
   ...
   #+NAME: send_post
   #+BEGIN_SRC lisp
     (defmacro send-post ((personal-url cookie-jar cookie-alist) &body body)
       `(drakma:http-request
         personal-url
...
*** Имя, возраст, город (=personal=)
    ...
    Сгенерируем из этой таблицы код, который формирует POST-запрос
    ...
    #+BEGIN_SRC lisp
      ...
      ;;   (print (personal cookie-jar (car (all-resume)))))
   #+END_SRC
...
*** Контакты (=contacts=)
    ...
    #+BEGIN_SRC lisp
      ...
      ;; (let ((cookie-jar (make-instance 'drakma:cookie-jar)))
      ;;   (print
      ;;    (contacts cookie-jar *test-resume*
