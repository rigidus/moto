...
** Планировка (flat)
   ...
   #+NAME: flat_data
     ...
     | area-sum     | (or db-null integer) | общая площадь квартиры (может быть дробное)    |
     | area-living  | (or db-null varchar) | жилая площадь квартиры (именно varchar)        |
     | area-kitchen | (or db-null integer) | площадь кухни (может быть дробное)             |
   ...
   (define-page cmpx "/cmpx/:cmpx-id"
     (let* ((i (parse-integer cmpx-id))
       ...
       (if (null u)
           ...
           (format nil "~{~A~}"
                   (list
                    (format nil "<h1>Страница жилого комплекса ~A</h1>" (id u))
                    (format nil "<h2>Данные комплекса ~A</h2>" (name u))
                    (tbl
                     (with-element (u u)
                       (row "Название" (name u))
                       (row "Адрес" (addr u)))
                    ...
                    (format nil "<h2>Очереди комплекса ~A</h2>~%~A"
                            (name u)
                            (tbl
                             (with-collection (i (funcall #'all-plex))
                               (tr
                                (td
                   ...
                   (list
                    ...
                    (tbl
                     ...
                     (format nil "<h2>Корпуса очереди жилого комплекса</h2>~%~A"
                            (tbl
                             (with-collection (i (funcall #'all-crps))
                               (tr
                                (td
...
** TODO Страница корпуса очереди ЖК
** TODO Страница планировки
