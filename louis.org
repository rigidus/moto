...
** Системное сообщение
   ...
   #+BEGIN_SRC lisp
     ...
     (defmacro system-msg ((&rest rest &key class &allow-other-keys) &body body)
       (let ((result-box-class "box system-message"))
         ...
         (let ((box `(:div :class ,result-box-class)))
           (unless (null rest)
             (setf box (append box rest)))
           `(ps-html (,box ((:span :class "result-icon result-icon--success media__item media__item--left"))
                           ((:div :class "system-message__text-container")
                            ((:div :class "system-message__text") ,@body))
                           ((:span :class "clear")))))))

     ;; (macroexpand-1 '(system-msg () "zzz"))
     ;; (system-msg () "zzz")
...
** Страница регистрации
   ...
   #+NAME: reg_valid_cond
     ...
     |------------------+-----------------------------------------------------------------+------------------------------------------------------------------|
     | mail             | (not (contains (get-val "mail")  "@"))                          | "Пожалуйста, введите корректный емайл"                           |
       ...
       (ps-html
        ((:script :type "text/javascript")
         (ps
           ...
           ;; <<gen_js_validator("reg-valid", reg_valid_cond)>>
           ))))
     ...
     (define-page reg "/reg"
       ...
       (:register (ps-html
                  ...
                  (macrolet ((get-val (selector)
                    ...
                    ;; <<gen_controller_validator("reg-valid", reg_valid_cond)>>
                    ;; (aif (reg-valid p)
                    ;;      ;; Возвращены ошибки
                    ;;      (dbg "~A" (bprint it))
