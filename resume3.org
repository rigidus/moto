...
#+BEGIN_SRC elisp :results value verbatim
  ...
  (let ((filename "~/repo/moto/resume4.org"))
    (let ((result ""))
      (with-temp-buffer
  ...
  (defun process-about-section (param)
    (mapcar #'(lambda (par)
                (let ((lead (car par)))
                  (cond ((equal lead 'paragraph)   (process-paragraph  (cdr par)))
                        ((equal lead 'plain-list)  (process-plain-list (cdr par)))
                        (t (message "process-about-section [unknown lead] %s" lead)))))
            param))

  (defun process-paragraph (param)
    (let ((meta (car param))
          (data (cdr param)))
      (mapcar #'(lambda (par)
                  (if (stringp par)
                      (setf result (format "%s%s" result par))
                    (let ((lead (car par)))
  ...
  (defun process-link (param)
    (let ((meta (car param))
          (data (cdr param)))
      (setf result (format "%s%s" result (getf meta :raw-link)))))

  (defun process-plain-list (param)
    (let ((meta (car param))
          (data (cdr param)))
      (mapcar #'(lambda (par)
                  (let ((lead (car par)))
                    (cond ((equal lead 'item)
                           (progn
                             (process-plain-list-item (cdr par))))
                          (t (message "process-plain-list [unknown lead] %s" lead)))))
              data)))

  (defun process-plain-list-item (param)
    (let ((meta (car param))
          (data (cdr param)))
